...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-10">
        <h2 class="text-4xl font-display text-textMain tracking-tight">Nueva Categoría</h2>
        <p class="text-gray-500 font-medium">Crea un grupo para organizar tus productos.</p>
    </div>
            ...
            <form method="POST" action="{{ route('categories.store') }}" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <input type="text" name="name" placeholder="Ej. Combos Navideños" class="w-full bg-gray-50 border border-gray-100 rounded-2xl py-4 px-6 text-textMain font-bold focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition" required>
                    </div>
                    <div>
                        ...
                        <div class="relative" id="icon-selector-container">
                            <input type="hidden" name="icon" id="icon-input-value" value="fas fa-pizza-slice">
                            <div id="icon-dropdown-trigger" class="w-full bg-gray-50 border border-gray-100 rounded-2xl py-4 px-6 flex items-center justify-between cursor-pointer hover:border-primary/50 transition">
                                <div id="icon-selected-preview" class="flex items-center gap-3">
                                    <i class="fas fa-pizza-slice text-primary text-xl"></i>
                                    <span class="font-bold text-textMain">fas fa-pizza-slice</span>
                                </div>
                                    ...
                                    @foreach($icons as $category => $list)
                                        <div class="mb-6 icon-category-group">
                                            <h4 class="text-[10px] font-bold text-gray-300 uppercase tracking-widest mb-4 ml-1 flex items-center gap-2">
                                                <span class="w-1 h-1 bg-primary rounded-full"></span> {{ $category }}
                                            </h4>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <input type="number" name="sort_order" value="0" class="w-full bg-gray-50 border border-gray-100 rounded-2xl py-4 px-6 text-textMain font-bold focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition" required>
                    </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4">
                        <input type="color" name="color" value="#00A859" class="h-14 w-14 rounded-2xl cursor-pointer shadow-sm border border-gray-100">
